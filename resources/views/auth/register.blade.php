    ...
    <div class="w-full max-w-[450px] bg-white rounded-2xl shadow-xl border border-slate-100 p-8 relative z-10">
        ...
        <form x-data="registerForm()" @submit.prevent="submitForm($event)" class="space-y-5" action="{{ route('register') }}" method="POST" id="regForm">
            ...
            <div>
                ...
                <p x-show="emailError" x-text="emailError" class="text-red-500 text-xs mt-1 font-medium" style="display: none;"></p>
            ...
            <div>
                ...
                <div class="relative" x-data="{ show: false }">
                    <input :type="show ? 'text' : 'password'" name="password" required 
                        class="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all placeholder-slate-400 sm:text-sm" placeholder="••••••••" minlength="8">
        ...
        function registerForm() {
            return {
                ...
                submitForm(e) {
                    ...
                    if (pwd.length < 8) {
                        alert('Keamanan Akun: Kata sandi terlalu pendek! Minimal harus 8 karakter.');
                        return;
                    }
