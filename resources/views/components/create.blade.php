...
dracoLoader.setDecoderPath('https://unpkg.com/three@0.160.0/examples/jsm/libs/draco/gltf/');
            ...
            window.loadModelIntoPreview = (url) => {
                console.log("DETEKTIF: Fungsi dipanggil dengan URL:", url);
                initThree();
                ...
                console.log("DETEKTIF: Memulai loader.load...");
                
                loader.load(
                    url, 
                    (gltf) => {
                        console.log("DETEKTIF: BERHASIL! GLTF sudah di tangan.");
                        // ... (isi kode sukses Anda)
                        if(loadingEl) loadingEl.style.display = 'none';
                    }, 
                    (xhr) => {
                        console.log("DETEKTIF: Sedang download... " + (xhr.loaded / xhr.total * 100).toFixed(2) + "%");
                    }, 
                    (err) => {
                        console.error("DETEKTIF: ERROR TOTAL! Pesan:", err.message);
                        console.error("DETEKTIF: Stack Trace:", err.stack);
                        alert("DETEKTIF ERROR: " + err.message);
                    }
                );
            };
